  ...
  <section class="flex flex-col lg:flex-row items-center justi space-x-6">
    ...
    <div>
      ...
      <div class="flex items-center space-x-4 mt-4 lg:mt-12">
        <button class="bg-teritary text-white px-3 py-1 rounded hover:bg-accent transition" onclick="edit_data_modal.showModal()">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
            <path fill="currentColor"
        ...
        @else
        <button onclick="modal_seller_register.showModal()" class="bg-teritary text-white px-3 py-1 rounded hover:bg-accent transition">
          Register As Seller
        </button>
        @include('buyer.includes.seller_regist_modal')
        @endif
        <a href="{{ route('buyer.logout') }}" class="bg-teritary text-white px-3 py-1 rounded hover:bg-red-600 transition text-center">
          Log Out
        </a>
      </div>
    </div>
